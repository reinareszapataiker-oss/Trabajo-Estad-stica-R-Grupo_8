...
sdnormal_typeI <- function(n, simulations, desvest, nmean, desvest_diff) {
  ...
  for (i in 1:simulations) {
    ...
    # Realizamos el test de la t y almacenamos el p-valor
    ...
    # Realizamos el test de wilcoxon y almacenamos el p-valor
    ...
    # Realizamos el test de normalidad (shapiro) y almacenamos el p-valor por
    # separado ya que cada grupo debe ser normal por separado en el caso que 
    # nos concierne. Si quisiéramos realizar un test de diferencia de medias
    # la normalidad debería aplicarse sobre el conjunto total de datos o directa-
    # mente sobre las diferencias de las medias
    pvalue1 <- shapiro.test(x1)$p.value
    pvalue2 <- shapiro.test(x2)$p.value
  ...
  normality_testTypeI <- normality_testFP / (simulations*2)
  t_testTypeI <- t_testFP / simulations
  wilcox_testTypeI <- wilcox_testFP / simulations
  
  
  
  results <- paste0("Test de normalidad: ", normality_testTypeI, "  |  ",
...
sdnormal_power <- function(n, simulations, effect = 3, desvest, desvest_diff) {
  ...
  # al total de simulaciomes
  normality_testTypeII <- normality_testFN / (simulations * 2)
  ...
  results <- paste0("Test de normalidad: ", normality_testPower, "  |  ",
